...
class PriorityQueue(Container):
    ...
    def add(self, item: Any) -> None:
        ...
        """
        # 因为后面要用到 event中的>, <, = 来表示所谓的大小，而与这里的str啥的无关
        # 此处 >= 是因为加入值一样，要让index往后排！
        # 往后排会使得Arrive后的 Checkout 这个添加的事件往后面的后面，
        # 即有可能出现在其他complete事件后，
        # 就会到达选择line加入->某列第一个人走->再处理check
        i = 0
...
if __name__ == '__main__':
    ...
    if check_pyta:
        ...
        python_ta.check_all(
            config={'allowed-import-modules': ['__future__', 'typing',
                                               'python_ta', 'doctest']}
        )
